...
npm run lint                         # Lint + check formatting (biome check .)
npm run lint:fix                     # Apply safe fixes (biome check --write .)
...
The project gate is `npm run typecheck && npm test && npm run lint` — all three must pass before completing a phase.

## Linting & Formatting (Biome)

- Configured in `biome.json` (Biome 2.x). Formatter: 2-space indent, double quotes, no semicolons, line width 110.
- Linter preset: `recommended`. `organizeImports` is enabled via `assist.actions.source` (applied by `npm run lint:fix`).
- NEVER run `biome check --unsafe ...`. Unsafe fixes can silently change semantics (e.g., `!` → `?.` in conditions) and broke `typecheck` before. Only use `--write` (safe fixes) or manual edits.
- To silence a justified rule locally, use a targeted `// biome-ignore lint/<group>/<rule>: reason` on the line above.
- No `any`-related complaints are enforced (`noExplicitAny`, `noImplicitAnyLet`, `noNonNullAssertion`, `noBannedTypes` are disabled) — the codebase intentionally uses `as any` in transform paths.
