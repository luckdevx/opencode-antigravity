...
import { getConfigDir, getDefaultLogsDir } from "./config/paths";
import { ensureGitignoreSync } from "./gitignore";
import { isDebugTuiEnabled as isDebugTuiEnabledState, setDebugTuiEnabled } from "./log-state";
import {
  deriveDebugPolicy,
...
export const DEBUG_MESSAGE_PREFIX = "[opencode-antigravity debug]";
...
function getLogsDir(customLogDir?: string): string {
  const logsDir = customLogDir || getDefaultLogsDir();
...
export function initializeDebug(config: AntigravityConfig): void {
  ...
  setDebugTuiEnabled(debugTuiEnabled);
...
export function logAccountContext(label: string, info: AccountDebugInfo): void {
  runWithDebugEnabled(() => {
    ...
    const indexLabel =
      info.index >= 0 ? `${info.index + 1}/${info.totalAccounts}` : `-/${info.totalAccounts}`;
