...
import { buildFingerprintHeaders, type Fingerprint, getSessionFingerprint } from "./fingerprint";
...
function extractConversationSeedFromContents(contents: any[]): string {
  ...
  const lastUser = users.length > 0 ? users[users.length - 1] : undefined;
  const primaryUser =
    firstUser && Array.isArray(firstUser.parts) ? extractTextFromContent(firstUser.parts) : "";
  if (primaryUser) {
    return primaryUser;
  }
...
function stripInjectedDebugFromParts(parts: unknown): unknown {
  ...
  return parts.filter((part) => {
    ...
    if (
      text &&
      (text.startsWith(DEBUG_MESSAGE_PREFIX) || text.startsWith(SYNTHETIC_THINKING_PLACEHOLDER.trim()))
    ) {
      return false;
    }
...
function ensureThinkingBeforeToolUseInContents(contents: any[], signatureSessionKey: string): any[] {
  return contents.map((content: any) => {
    ...
    if (!lastThinking) {
      ...
      log.debug("Stripping thinking from tool_use content (no valid cached signature)", {
        signatureSessionKey,
      });
...
export function prepareAntigravityRequest(
  ...
  if (typeof baseInit.body === "string" && baseInit.body) {
    {
      ...
      if (isWrapped) {
        ...
        if (isClaude) {
          configureClaudeToolConfig(requestPayload);
        }
        ...
        const effectiveUserThinkingConfig =
          isClaudeSonnetNonThinking || isImageModel ? undefined : userThinkingConfig;
        ...
        if (isImageModel) {
          ...
          if (normalizedThinking) {
            ...
            if (isClaudeThinking) {
              ...
              thinkingConfig = buildClaudeThinkingConfig(
        ...
        if (hasTools) {
          if (isClaude) {
            // Claude tool normalization (full function declarations with schemas)
            const claudeResult = normalizeClaudeTools(requestPayload, cleanJSONSchemaForAntigravity);
            toolDebugMissing = claudeResult.toolDebugMissing;
            toolDebugSummaries.push(...claudeResult.toolDebugSummaries);
          } else {
  ...
  if (headerStyle === "antigravity") {
    ...
    const selectedHeaders = getRandomizedHeaders("antigravity");
