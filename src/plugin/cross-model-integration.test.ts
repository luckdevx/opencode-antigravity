...
import { getModelFamily, sanitizeCrossModelPayload } from "./transform/cross-model-sanitizer";
...
describe("Cross-Model Session Integration", () => {
  describe("Gemini → Claude model switch with tool calls", () => {
    ...
    it("preserves non-signature metadata", () => {
      ...
      const sanitized = result.payload as typeof payload;
      const partMeta = (sanitized.contents[0]!.parts![0] as Record<string, unknown>).metadata as Record<
        string,
        unknown
      >;
  ...
  describe("Claude → Gemini model switch", () => {
    it("sanitizes Claude thinking blocks when preparing Gemini request", () => {
      ...
      const toolBlock = content.find((c: Record<string, unknown>) => c.type === "tool_use") as Record<
        string,
        unknown
      >;
  ...
  describe("Same model family - no sanitization needed", () => {
    it("preserves Gemini signatures when staying on Gemini", () => {
      ...
      const sanitized = result.payload as typeof payload;
      expect((sanitized.contents![0]!.parts![0] as Record<string, unknown>).thoughtSignature).toBe(
        "valid-gemini-sig",
      );
    ...
    it("preserves Claude signatures when staying on Claude", () => {
      ...
      const sanitized = result.payload as typeof payload;
      expect((sanitized.messages![0]!.content![0] as Record<string, unknown>).signature).toBe(
        "valid-claude-sig",
      );
      expect(result.modified).toBe(false);
    });
  ...
  describe("Edge cases", () => {
    ...
    it("handles wrapped requests array (batch format)", () => {
      ...
      expect(
        (sanitized.requests![1]!.contents![0]!.parts![0] as Record<string, unknown>).metadata,
      ).toBeUndefined();
      expect(result.signaturesStripped).toBe(2);
    });
