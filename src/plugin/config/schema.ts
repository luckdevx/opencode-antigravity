/**
 * Configuration schema for opencode-antigravity plugin.
 *
...
export const AntigravityConfigSchema = z.object({
   ...
   */
  signature_cache: SignatureCacheConfigSchema.default(SignatureCacheConfigSchema.parse({})),
  ...
  /**
   * List of model keys (as they appear in opencode.json `provider.google.models`)
   * to hide from the model picker AND the quota display.
   *
   * When non-empty, "Configure models in opencode.json" only writes the
   * non-hidden models, and "Check quotas" only shows quota for models whose
   * canonical base name matches a non-hidden model. This keeps the model list
   * and the quota view consistent: only models you can actually use appear.
   *
   * Base-name matching strips `antigravity-`, `-preview-customtools`,
   * `-preview`, and thinking-tier suffixes (`-low`/`-medium`/`-high`/`-minimal`),
   * so hiding `gemini-3-pro-preview` also hides upstream `gemini-3-pro-low`
   * while keeping `gemini-3.1-pro-*` (a different base).
   *
   * Example (hide the deprecated gemini-3-pro variants):
   *   "hidden_models": [
 ...
 *
 * Derived directly from the Zod schema so defaults can never drift from the
 * schema definitions (all fields declare their own `.default(...)`).
 */
export const DEFAULT_CONFIG: AntigravityConfig = AntigravityConfigSchema.parse({});
