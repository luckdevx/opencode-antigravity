...
import { createThoughtBuffer, deduplicateThinkingText } from "./core/streaming/transformer";
import {
...
describe("extractThinkingConfig", () => {
  ...
  it("uses default budget for Anthropic-style without budgetTokens", () => {
    const result = extractThinkingConfig({ thinking: { type: "enabled" } }, undefined, undefined);
  ...
  it("uses default budget when thinkingBudget not specified", () => {
    const result = extractThinkingConfig({}, { thinkingConfig: { includeThoughts: true } }, undefined);
    expect(result).toEqual({ includeThoughts: true, thinkingBudget: DEFAULT_THINKING_BUDGET });
  });
});
...
describe("filterUnsignedThinkingBlocks", () => {
  ...
  it("filters Gemini-style thought parts with short signatures", () => {
    const contents = [
      {
        role: "model",
        parts: [{ thought: true, text: "has short signature", thoughtSignature: "sig" }],
      },
    ];
    const result = filterUnsignedThinkingBlocks(contents);
  ...
  it("preserves tool_use and tool_result blocks intact", () => {
    const contents = [
      {
        role: "model",
        parts: [{ type: "tool_use", id: "tool_123", name: "bash", input: { command: "ls" } }],
      },
      {
        role: "user",
        parts: [{ type: "tool_result", tool_use_id: "tool_123", content: "file1.txt" }],
      },
    ];
    const result = filterUnsignedThinkingBlocks(contents);
    expect(result[0].parts[0]).toEqual({
  ...
  it("preserves nested tool_result format", () => {
    const contents = [
      {
        role: "user",
        parts: [{ tool_result: { tool_use_id: "tool_789", content: "nested result" } }],
      },
    ];
    const result = filterUnsignedThinkingBlocks(contents);
    expect(result[0].parts).toHaveLength(1);
  ...
  it("transforms Gemini-style candidates", () => {
    const response = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "thinking here" }, { text: "output" }],
          },
        },
      ],
    };
  ...
  it("does not add providerMetadata when no signature present", () => {
    const response = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "thinking without signature" }, { text: "output" }],
          },
        },
      ],
    };
  ...
  describe("enum merging from anyOf/oneOf", () => {
    ...
    it("merges oneOf with const values into enum", () => {
      const schema = {
        ...
        properties: {
          status: {
            oneOf: [{ const: "pending" }, { const: "active" }, { const: "completed" }],
          },
        },
      };
    ...
    it("merges anyOf with single-value enums into combined enum", () => {
      const schema = {
        ...
        properties: {
          level: {
            anyOf: [{ enum: ["low"] }, { enum: ["medium"] }, { enum: ["high"] }],
          },
        },
      };
  ...
  it("filters out empty delta parts", () => {
    ...
    const chunk2 = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "Complete thought" }, { text: "Regular text" }],
          },
        },
      ],
    };
