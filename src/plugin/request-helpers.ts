...
function cleanupRequiredFields(schema: any): any {
  ...
  if (Array.isArray(result.required) && result.properties && typeof result.properties === "object") {
    const validRequired = result.required.filter((req: string) => Object.hasOwn(result.properties, req));
...
function addEmptySchemaPlaceholder(schema: any): any {
  ...
  if (isObjectType) {
    const hasProperties =
      result.properties && typeof result.properties === "object" && Object.keys(result.properties).length > 0;
  ...
  if (google) {
    ...
    if (typeof google.thinkingLevel === "string") {
      ...
      result.includeThoughts =
        typeof google.includeThoughts === "boolean" ? google.includeThoughts : undefined;
