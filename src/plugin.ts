...
import { buildAuthSuccessFromStoredAccount, persistAccountPool } from "./plugin/account-pool";
import {
  AccountManager,
  calculateBackoffMs,
...
} from "./plugin/rate-limit-helpers";
import {
...
import { createSyntheticErrorResponse, isEmptyResponseBody } from "./plugin/request-helpers";
...
// Progressive rate limit retry delays and backoff state now live in
// ./plugin/rate-limit-state (imported at the top of this file).
      ...
      if (sessionRecovery && input.event.type === "session.error") {
        ...
        if (sessionRecovery.isRecoverableError(error)) {
          ...
          if (recovered && sessionID && config.auto_resume) {
            ...
            await showToastMessage(client, successToast.message, "success", {
      ...
      auth: {
        ...
        loader: async (
          ...
          if (isDebugEnabled()) {
            ...
            if (logPath) {
              await showToastMessage(client, `Debug log: ${logPath}`, "info");
            }
          }
          ...
          return {
            ...
            async fetch(input, init) {
              ...
              const showToast = async (
                ...
                log.debug("toast", { message, variant, isChildSession, toastScope, childSessionParentID });

                await showToastMessage(client, message, variant, {
                  quietMode,
                  toastScope,
                  isChildSession,
                  abortSignal,
              ...
              while (true) {
                ...
                if (!account) {
                  ...
                  const waitMs =
                    accountManager.getMinWaitTimeForFamily(family, model, preferredHeaderStyle, strictWait) ||
                    60_000;
                ...
                if (accountManager.isRateLimitedForHeaderStyle(account, family, headerStyle, model)) {
                  ...
                  if (allowQuotaFallback && family === "gemini" && headerStyle === "antigravity") {
                    ...
                    if (
                      accountManager.hasOtherAccountWithAntigravityAvailable(account.index, family, model)
                    ) {
                    ...
                    try {
                      ...
                      if (response.status === 429 || response.status === 503 || response.status === 529) {
                        ...
                        if (accountCount > 1) {
                          ...
                          const expBackoffMs = Math.min(FIRST_RETRY_DELAY_MS * 2 ** (attempt - 1), 60000);
                      ...
                      if (response.ok && !prepared.streaming) {
                        ...
                        if (isEmptyResponseBody(bodyText)) {
                          ...
                          const currentAttempts = incrementEmptyResponseAttempts(emptyAttemptKey);
                          ...
                          clearEmptyResponseAttempts(emptyAttemptKey);
                        ...
                        clearEmptyResponseAttempts(emptyAttemptKeyClean);
                      }
              ...
              if (inputs) {
                ...
                if (existingStorage && existingStorage.accounts.length > 0) {
                  ...
                  while (true) {
                    ...
                    if (menuResult.mode === "verify" || menuResult.mode === "verify-all") {
                      ...
                      if (verification.status === "ok") {
                        const { changed, wasVerificationRequired } = clearStoredAccountVerificationRequired(
                          account,
                          true,
                        );
                        if (changed) {
                    ...
                    if (updatedAccounts.length > 0) {
                      ...
                      if (fallbackAccount?.refreshToken) {
                        ...
                        try {
                          ...
                          log.error("Failed to update stored Antigravity OAuth credentials", {
                            error: String(storeError),
                          });
                        }
                ...
                while (accounts.length < MAX_OAUTH_ACCOUNTS) {
                  ...
                  await showToastMessage(
                    client,
                    `Account ${accounts.length} authenticated${result.email ? ` (${result.email})` : ""}`,
                    "success",
                  );
                  ...
                  callback: async (): Promise<AntigravityTokenExchangeResult> => {
                    ...
                    try {
                      ...
                      if (result.type === "success") {
                        ...
                        await showToastMessage(client, toastMessage, "success");
                      }
