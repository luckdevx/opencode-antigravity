...
### Model Availability Filtering (`hidden_models`)

Models come from a hardcoded manifest — including deprecated ones like `gemini-3-pro` that no longer work. A configurable `hidden_models` option in `antigravity.json` lets you hide the ones you don't use:
...
When configured:
- **Model picker** (`opencode.json`) only includes models you can actually use
- **Quota display** (`opencode auth login` → Check quotas) only shows quota for non-hidden models
- Both views stay **consistent** — no more confusion from models that appear but fail on invocation
...
**This plugin is loaded from a local path (not npm).**
...
2. **Add the plugin** to `~/.config/opencode/opencode.json` (or `opencode.jsonc`):
...
4. **Configure hidden models** (optional) — create `~/.config/opencode/antigravity.json`:
