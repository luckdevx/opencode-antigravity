...
```
src/
├── plugin.ts                  # Main entry, fetch interceptor, OAuth flows
├── constants.ts               # Endpoints, headers, config
├── antigravity/
│   └── oauth.ts               # OAuth token exchange
└── plugin/
    ├── auth.ts                # Token validation & refresh (PackedRefresh)
    ├── account-pool.ts        # Account merge/persist after login
    ├── env.ts                 # WSL/remote detection, browser opening
    ├── warmup.ts              # Session warmup tracking
    ├── url-routing.ts         # Model/header routing from request URL
    ├── rate-limit-state.ts    # In-memory backoff/failure/empty-response state
    ├── rate-limit-helpers.ts  # Retry-after & duration parsing
    ├── toast.ts               # Centralized toast notifications
    ├── gitignore.ts           # .gitignore management for config dir
    ├── log-state.ts           # Shared TUI logging flag
    ...
    ├── cache.ts               # Auth & signature caching (memory + disk)
    ├── cache/
    │   └── signature-cache.ts # Disk-based signature persistence
    ├── config/
    │   ├── schema.ts          # Zod config schema
    │   ├── loader.ts          # Config file loading
    │   └── paths.ts           # Centralized config directory paths
