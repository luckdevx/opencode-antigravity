...
function generateEnduranceTest(turnCount: number): TurnConfig[] {
  ...
  for (let i = 0; i < turnCount; i++) {
    ...
    const prompt = template.prompt.replace(/\{i\}/g, String(i + 1)).replace(/\{n\}/g, String(i + 1));
...
const RATE_LIMIT_ERROR_PATTERNS = ["false alarm", "incorrectly marked as rate limited", "wrong quota"];
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      ...
      if (!extractedSessionId) {
        const match =
          stdout.match(/session[:\s]+([a-zA-Z0-9_-]+)/i) || stderr.match(/session[:\s]+([a-zA-Z0-9_-]+)/i);
        if (match) {
...
async function runMultiTurnTest(test: MultiTurnTest): Promise<TestResult> {
  ...
  for (let index = 0; index < test.turns.length; index++) {
    ...
    const result = await runTurn(prompt, model, sessionId ?? null, `regression-${test.name}`, test.timeout);
